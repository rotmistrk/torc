...
Parser& Parser::flag(bool* dest, char short_name,
...
Parser& Parser::option(std::string* dest, char short_name,
...
std::string Parser::build_short_opts() const {
    ...
    for (const auto& f : flags_) {
        if (f.short_name) s += f.short_name;
    }
    for (const auto& o : options_) {
        if (o.short_name) { s += o.short_name; s += ':'; }
    }
    s += "hV";
    return s;
}
...
std::vector<struct option> Parser::build_long_opts() const {
    ...
    for (const auto& f : flags_) {
        opts.push_back({f.long_name.c_str(), no_argument, nullptr,
                        f.short_name ? f.short_name : 0});
    }
    for (const auto& o : options_) {
        opts.push_back({o.long_name.c_str(), required_argument, nullptr,
                        o.short_name ? o.short_name : 0});
    }
...
int Parser::parse(int argc, char** argv) {
    ...
    while ((ch = getopt_long(argc, argv, short_opts.c_str(),
        ...
        if (ch == 'h') { print_help(); return EX_OK; }
        if (ch == 'V') { print_version(); return EX_OK; }

        bool matched = false;
        for (auto& f : flags_) {
            if (f.short_name == ch) { *f.dest = true; matched = true; break; }
        }
        if (!matched) {
            for (auto& o : options_) {
                if (o.short_name == ch) {
