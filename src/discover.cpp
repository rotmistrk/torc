...
bool discover_deps(const Package &pkg, const std::string &prefix, const DiscoverVisitor &visitor) {
    ...
    std::string cmd =
        pkg.discover() + " '" + pkg.name() + "' '" + pkg.version() + "' '" + prefix + "'";
    auto output = run_script(cmd);
    if (output.empty())
        return true; // no deps or script failed silently
    ...
    for (const auto &item : root.as_list()) {
        ...
        if (auto *n = item.get("name"))
            dep.set_name(n->as_scalar());
        if (auto *v = item.get("version"))
            dep.set_version(v->as_scalar());
        if (auto *s = item.get("source"))
            dep.set_source(s->as_scalar());
        if (auto *c = item.get("sha256"))
            dep.set_sha256(c->as_scalar());
        if (auto *b = item.get("build"))
            dep.set_build(b->as_scalar());
        if (auto *d = item.get("discover"))
            dep.set_discover(d->as_scalar());
        if (auto *l = item.get("lib"))
            dep.set_lib_name(l->as_scalar());
        if (dep.lib_name().empty())
            dep.set_lib_name(dep.name());
        if (!dep.name().empty())
            visitor(std::move(dep));
    }
    return true;
}
