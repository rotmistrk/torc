...
static bool needs_rebuild(const std::string &src, const std::string &obj) {
    ...
    if (fs::last_write_time(src) > obj_time)
        return true;
    ...
    while (std::getline(in, line)) {
        ...
        while (iss >> dep) {
            if (dep == "\\")
                continue;
            if (fs::exists(dep) && fs::last_write_time(dep) > obj_time)
                return true;
        }
    }
    return false;
...
static std::string build_cxxflags(const Manifest &m, const BuildOpts &opts) {
    ...
    if (!opts.extra_cxxflags().empty())
        flags += " " + opts.extra_cxxflags();
    if (!opts.toolchain().empty()) {
        auto *tc = m.find_toolchain(opts.toolchain());
        if (tc && !tc->cxxflags().empty())
            flags += " " + tc->cxxflags();
    }
...
static int compile_sources(const std::vector<std::pair<std::string, std::string>> &to_compile,
    ...
    if (fail_rc != EX_OK)
        diag::error("build", "compilation failed: " + fail_name);
    return fail_rc;
}

static int link_target(const std::vector<std::string> &all_objs, const std::string &target_path,
                       const std::string &cxx, const std::string &cxxflags,
                       const std::string &ldflags, const std::string &libs) {
...
int cmd_build(const Manifest &m, const BuildOpts &opts) {
    ...
    for_each_source(opts.src_dir(), opts.recursive(),
                    [&](const std::string &p) { srcs.push_back(p); });
    if (srcs.empty()) {
    ...
    std::string target_name =
        opts.target().empty() ? fs::current_path().filename().string() : opts.target();
    ...
    for (const auto &src : srcs) {
        ...
        if (needs_rebuild(src, obj))
            to_compile.emplace_back(src, obj);
    }
    ...
    return link_target(all_objs, target_path, cxx, cxxflags, build_ldflags(m), build_libs(m));
}
