...
void error(std::string_view context, std::string_view msg) {
    std::fprintf(stderr, "%storc: error:%s %.*s: %.*s\n", red(), reset(),
                 static_cast<int>(context.size()), context.data(), static_cast<int>(msg.size()),
                 msg.data());
}
...
void info(std::string_view msg) {
    std::fprintf(stderr, "%storc:%s %.*s\n", green(), reset(), static_cast<int>(msg.size()),
                 msg.data());
}
...
void progress(std::string_view label, int current, int total) {
    if (!is_terminal())
        return;
    std::fprintf(stderr, "\r%storc:%s %.*s [%d/%d]", green(), reset(),
                 static_cast<int>(label.size()), label.data(), current, total);
    std::fflush(stderr);
}
