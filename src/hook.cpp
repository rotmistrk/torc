...
static const char *TORC_BLOCK = "# ── BEGIN torc ─────────────────────────\n"
                                "-include extdep.mak\n"
...
static void write_replaced(std::ofstream &out, const std::vector<std::string> &lines, int begin_idx,
                           int end_idx) {
...
static void write_inserted(std::ofstream &out, const std::vector<std::string> &lines) {
    int insert_at = 0;
    for (int i = 0; i < static_cast<int>(lines.size()); ++i) {
        auto &l = lines[static_cast<size_t>(i)];
        bool is_rule = !l.empty() && l[0] != '#' && l[0] != ' ' && l[0] != '\t' &&
...
int cmd_hook(const HookOpts &opts) {
    ...
    for (int i = 0; i < static_cast<int>(lines.size()); ++i) {
        if (lines[i].find(BLOCK_BEGIN) != std::string::npos)
            begin_idx = i;
        if (lines[i].find(BLOCK_END) != std::string::npos)
            end_idx = i;
    }

    fs::copy_file(opts.makefile(), opts.makefile() + ".bak", fs::copy_options::overwrite_existing);
