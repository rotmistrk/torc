...
static void rewrite_manifest(const std::string &path, const std::vector<VersionInfo> &updates) {
    ...
    for (const auto &u : updates) {
        if (u.latest().empty() || u.latest() == u.current())
            continue;
        size_t pos = 0;
...
static std::vector<VersionInfo>
check_packages(const Manifest &m, const std::vector<std::unique_ptr<VersionChecker>> &checkers) {
...
int cmd_update(const Manifest &m, const UpdateOpts &opts, const std::string &manifest_path) {
    ...
    for (const auto &vi : infos)
        if (!vi.latest().empty() && vi.latest() != vi.current())
            ++updates;
